...
> **Après avoir cloné le dépôt :** l'environnement virtuel (`venv/`) et la base
> de données (`db.sqlite3`) **ne sont pas inclus** dans le dépôt — il faut les
> recréer avec les étapes ci-dessous. `pip install` seul ne suffit pas : les
> étapes 3 (`migrate`) et 4 (`seed.py`) créent la base et les données.
...
python seed.py               # données de démo + compte admin
